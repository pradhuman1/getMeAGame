...
document.querySelector('.chat-area').addEventListener('click', e => {
  ...
  if (flag === "smallCard") {
    if (obj[gameID]) {
      ...
      if (globalData) obj[gameID] = globalData;
    } // console.log(obj);
    ...
    if (globalData) {
      ...
      var currImg = 0;
      var store;
      if (game.clip) videoImage.push(game.clip);
      game.screenshots.forEach(curr => {
        videoImage.push(curr.image);
      });
      removePreviousStuff();
      modifyCard(game); //Adding url to buy button

      document.querySelector('.stores').addEventListener('click', e => {
        var url = e.target.id;
        if (url) document.querySelector('.buy').action = url;
      }); //Removing popup
      ...
      document.querySelector('.previewCards').addEventListener('click', e => {
        if (e.target.id === "image") displayOnScreen(e.target.src, e.target.id);else if (e.target.id === "video") displayOnScreen("", e.target.id);
      }); //chnging preview Cards

      document.querySelector('.btn-right').addEventListener('click', changePreviewRight);
      document.querySelector('.btn-left').addEventListener('click', changePreviewLeft); //For star reviews
      ...
      document.querySelector('.stores').addEventListener('click', e => {
        ...
        if (store) document.getElementById(store).classList.add("selectedStore");
      ...
      function modifyCard(game) {
        ...
        document.querySelector('.image').insertAdjacentHTML('beforeend', html); //adding stores

        game.stores.forEach(curr => {
          html = '<div class="store" id="%storeUrl%" >%storeName%</div>';
          html = html.replace("%storeName%", curr.store.name); //Changing url for indian stores
        ...
        document.querySelector('.rating-count').textContent = game.rating_count + " Ratings"; //adding screenshots and video

        if (game.clip) displayOnScreen(videoImage[0], "video");else {
      ...
      function displayPreview(starting) {
        ...
        for (var i = starting; i < starting + 4 && videoImage.length; i++) {
          if (i === 0 && game.clip) html = '<div class="preview-card" ><img src="./resources/video-placeholder.jpg" width="128px" height="98px"  id="video"></div>';else {
